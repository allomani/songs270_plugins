...
if(!check_login_cookies()){die("<center> $phrases[access_denied] </center>");} 
    ...
    //--------- comments del ----
if($action=="photos_comment_del"){
    if_admin("photos");  
    db_query("delete from photos_comments where id='$id'");
 print "<SCRIPT>window.location=\"$scripturl/index.php?action=photos_preview&id=$cat\";</script>";
 }
...
if ($add_cat){
         ...
         db_query("insert into photos_cats(name,cat,img) values('$name','$cat','$img')");

        }
 //================================================
 if($action=="photos_edit_cat_ok"){

       db_query("update photos_cats set name='$name',img='$img' where id='$id'");
         }
//--------------------------- File Delete Query ------------------------------------
...
$thumb_saved =  create_thumb($file_saved,$settings['photos_thumb_width'],$settings['photos_thumb_hieght'],0);
...
print "<p align=$global_align><img src='images/link.gif'><a href='index.php?action=photos_cats&cat=0'>$phrases[main_page] </a> / $dir_content</p>";
 print "<br><table width=70% class=grid><tr><td>";
       ...
       print "<center><img src='".get_image($qr_title['img'],'../images/folder.gif')."'><br>$qr_title[name]</center>" ;
            }
...
$qr = db_query("select * from photos_data where cat='$cat'");
...
if($c == 4){
      ...
      print "
       <td align=center>
      <img src='$scripturl/".get_image($data['thumb'])."'><br>$data[name]<br>
