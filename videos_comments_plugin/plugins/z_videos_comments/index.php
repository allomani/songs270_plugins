...
if($action=="preview"){
   ...
   if($sec_img->verify_string($sec_string)){
            ...
            db_query("insert into songs_videos_comments (userid,content,cat,active,date) values('$userid','$content','$id','".iif($auto_activate_videos_comments,1,0)."',now())");
                                         
            open_table();
            if($auto_activate_videos_comments){
     ...
     //-------------- Comments --------------------
$qr = db_query("select * from songs_videos_comments where cat ='$id' and active=1");
  if(db_num($qr)){
          open_table("«· ⁄·Ìﬁ« ");
          ...
          if(check_login_cookies() && if_admin("videos_comments",1)){
          print " &nbsp;[<a href='".iif($admin_folder,$admin_folder,"admin")."/index.php?action=videos_comment_del&id=$data[id]&cat=$id'>Õ–›</a>]";
              }
