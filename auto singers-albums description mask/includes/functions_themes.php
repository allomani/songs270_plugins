...
$meta_description = str_replace(array("{singer}","{album}"),array($data['name'],$data_album['name']),get_template('albums_description')); 

}else{
    $meta_description = str_replace("{singer}",$data['name'],get_template('singers_description'));   
}
        }else{
...
$data = db_fetch($qr) ;
$title_sub = "$data[name]" ;
$meta_description = str_replace("{cat}",$data['name'],get_template('cats_description'));  
        }else{
 $title_sub = "" ;
 }
 }
