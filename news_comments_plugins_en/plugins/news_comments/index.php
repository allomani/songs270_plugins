...
if($action=="news" && $id){
   ...
   if($sec_img->verify_string($sec_string)){
            ...
            db_query("insert into songs_news_comments (userid,content,cat,active,date) values('$userid','$content','$id','1',now())");
     ...
     //-------------- Comments --------------------
$qr = db_query("select * from songs_news_comments where cat ='$id' and active=1");
  if(db_num($qr)){
          open_table("Comments");
          ...
          if(check_login_cookies() && if_admin("news_comments",1)){
          print " &nbsp;[<a href='".iif($admin_folder,$admin_folder,"admin")."/index.php?action=news_comment_del&id=$data[id]&cat=$id'>Delete</a>]";
              }
